...
class GzhFragment : Fragment() {
    ...
    private val gzhData = ArrayList<Datas>()
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        val gzhAdapter= ArticleAdapter(gzhData, this.requireActivity(), 0)
        val loadMoreAdapter =
            ...
            animate().alpha(1f)
                ...
                .setListener(object : AnimatorListenerAdapter() {
                    override fun onAnimationEnd(animation: Animator?) {
                        tv_gzh_update_in.visibility = View.GONE
                        tvOnAnimation()
                    }
                })
        }
    ...
    fun tvOnAnimation() {   // tv持续动画
        tv_gzh_update_on.visibility = View.VISIBLE
        tv_gzh_update_on.apply {
            alpha = 1f
            animate().alpha(1f)
                .setDuration(800)
                .setListener(object : AnimatorListenerAdapter() {
                    override fun onAnimationEnd(animation: Animator?) {
                        tv_gzh_update_on.visibility = View.GONE
                        tvOutAnimation()
                    }
                })
        }
    ...
    fun tvOutAnimation() {  // tv淡出动画
        tv_gzh_update_out.visibility = View.VISIBLE
        tv_gzh_update_out.apply {
            alpha = 1f
            animate().alpha(0f)
                .setDuration(500)
                .setListener(object : AnimatorListenerAdapter() {
                    override fun onAnimationEnd(animation: Animator?) {
                        tv_gzh_update_out.visibility = View.GONE
                    }
                })
        }
    }
